...
function BallisticBullet:_getnewPosition(delta)
	return self.pos + ( self.velocity + TFA.Ballistics.Gravity * delta * 0.5 ) * delta--verlet
end
...
function BallisticBullet:_moveSafe(newPos)
	...
	--collision trace
	if self.playerOwned then self.owner:LagCompensation(true) end
	util.TraceLine(traceData)
	if self.playerOwned then self.owner:LagCompensation(false) end
	--collision check
	if traceRes.Hit and traceRes.Fraction<1 and traceRes.Fraction>0 then
...
function BallisticBullet:Impact(tr)
	...
	setmetatable(bul,{ ["__index"] = self.bul })
...
function BallisticBullet:Render()
	...
	if IsValid(self.curmodel) then
		if cv_bullet_style:GetBool() or self.smokeparticle~="" then
			local fpos,fang
...
function TFA.Ballistics:Bullet(t)
	...
	setmetatable(b,{ ["__index"] = BallisticBullet })
	return b
end