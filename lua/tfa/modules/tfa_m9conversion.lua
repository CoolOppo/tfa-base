...
CreateConVar("sv_tfa_conv_m9konvert","0",{FCVAR_ARCHIVE,FCVAR_REPLICATED,FCVAR_SERVER_CAN_EXECUTE},"Convert M9K to TFA at runtime?")
...
function TFABaseConv_IsScoped( tbl )
	...
	if tbl.Scoped or
	tbl.IsScoped or
	( tbl.ScopeZoom and tbl.ScopeZoom>0) or
	tbl.ScopeZooms or
	(tbl.Secondary and tbl.Secondary.ScopeZoom and tbl.Secondary.ScopeZoom>0) or
	string.find(pn,"scope")	or
	string.find(cl,"scope")	or
	string.find(cat,"scope") or
	string.find(bn,"scope") or
	string.find(bn,"snip") or
	string.find(bn,"rifleman")
	then
		return true
	end
	return false
	...
	['m9k_dbarrel'] = {
		...
		['SecondaryAttack'] = function(self)
			if IsValid(self) and self.OwnerIsValid and self:OwnerIsValid() then 
				self:PrimaryAttack() self:PrimaryAttack()
			end
		end,
...
function TFABaseConv_IsRevolver( tbl )
	...
	if string.find(pn,"mag") then
		if string.find(pn,"44") or string.find(pn,"357") then
			return true
		end
	elseif string.find(cl,"mag") then
		if string.find(cl,"44") or string.find(cl,"357") then
			return true
		end
	elseif string.find(pn,"colt") then
		if string.find(pn,"python") or string.find(pn,"357") or string.find(pn,"44") then
			return true
		end
	elseif string.find(cl,"colt") then
		if string.find(cl,"357") or string.find(cl,"44") then
			return true
		end
...
function TFABaseConv_ism9k( tbl )
	if tbl.Primary.KickDown!=nil and ( ( tbl.Primary and tbl.Primary.IronAccuracy ) or (tbl.data and tbl.data.ironsights==0) ) and !string.find(tbl.Base or "","tfa_") then return true end
	return false
end

function tfaconvsingle_M9K( cn )
	if GetConVarNumber("sv_tfa_conv_m9k",0)<0.99 then return end
	...
	if tbl and tbl.Base then
		...
		if TFABaseConv_ism9k( tbl ) then
			...
			if string.find(base,"knife") or
			string.find(printn,"knife") or
			string.find(printn,"melee") or
			string.find(catn,"knife") or
			string.find(catn,"melee") or
			string.find(printn,"sword") or
			string.find(holdt,"knife") or
			string.find(holdt,"melee") or
			string.find(holdt,"fist") then
				tbl.data = {}
			...
			if !isshotgun and !tbl.BoltAction and !tbl.Revolver then
...
function tfa_m9k_main()
	if !weapons then return end
	local weaponlist = weapons.GetList()
	if !weaponlist then return end
	for k,v in pairs(weaponlist) do
		if v and v.ClassName and !string.find(v.ClassName,"_base") then
