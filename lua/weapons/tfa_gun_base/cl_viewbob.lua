...
local function l_Sign(x)
	if x ==0 then return 0 else return (x>0) and 1 or -1 end
end
local function l_ABSMax(x,y)
	return math.max( math.abs(x), math.abs(y) ) * l_Sign(x)
end
...
local procedural_pitchrestorefac = 7.5
...
function SWEP:CalcView(ply, pos, ang, fov)
	...
	if self.lastact == ACT_VM_HOLSTER or self.lastact == ACT_VM_HOLSTER_EMPTY then holprog = math.max( holprog,vm:GetCycle() ) end
	if self.lastact == ACT_VM_DRAW or self.lastact == ACT_VM_DRAW_EMPTY or self.lastact == ACT_VM_DRAW_SILENCED then holprog = math.max( holprog, math.Clamp(0.2-vm:GetCycle(),0,1)*5 ) end
	...
	if !ang or !pos then return oldangtmp,oldpostmp end

	if self.CameraAngCache then
		ang:RotateAroundAxis(ang:Right(),( self.CameraAngCache.p + self.CameraOffset.p )*viewbobintensity*5)
		ang:RotateAroundAxis(ang:Up(),( self.CameraAngCache.y + self.CameraOffset.y )*viewbobintensity*5)
		ang:RotateAroundAxis(ang:Forward(),( self.CameraAngCache.r + self.CameraOffset.r )*viewbobintensity*3)
	else
		...
		if angpos then
			...
			mzang_fixed = vm:WorldToLocalAngles( angpos.Ang )-- - self.MZReferenceAngle--WorldToLocal( angpos.Pos, angpos.Ang, angpos.Pos, oldangtmp + self.MZReferenceAngle )
		...
		if mzang_fixed_last then
			...
			delta:Normalize()
			mzang_velocity = mzang_velocity + delta * ( 2 * (1-self.ViewHolProg) )--* progress )
